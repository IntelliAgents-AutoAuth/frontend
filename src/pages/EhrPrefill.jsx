    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-[#38A3A5]/20 transition-all shadow-sm gap-4">
      <div className="flex flex-col gap-1">
        ...
        {hasData ? (
          isLabs ? (
            <div className="flex flex-wrap gap-1.5 mt-1">
              {Object.keys(value).map(k => (
                 <span key={k} className="px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700">
                   {k}
                 </span>
              ))}
            </div>
          ...
          <span className="text-sm font-medium text-slate-300 italic">Pending manual entry</span>
        )}
      </div>
      ...
      <div className="w-full max-w-4xl bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col md:flex-row z-10 transition-all duration-300">
        ...
        <div className="w-full md:w-[35%] bg-slate-50/50 p-8 sm:p-10 border-r border-slate-100 flex flex-col relative">
          <div className="mb-10">
            ...
            <h2 className="text-2xl font-extrabold text-slate-800 tracking-tight font-outfit" style={{ fontFamily: "'Outfit', sans-serif" }}>EHR Integration</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Case: {id}</p>
          </div>
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="space-y-4 pt-2">
              <div className={`flex items-center gap-3 text-sm font-semibold transition-all duration-500 ${progress >= 30 ? 'text-slate-800' : 'text-slate-300'}`}>
                <CheckCircle2 size={18} className={progress >= 30 ? 'text-[#38A3A5]' : 'text-slate-200'} />
                <span>Link FHIR Metadata</span>
              </div>
              <div className={`flex items-center gap-3 text-sm font-semibold transition-all duration-500 ${progress >= 60 ? 'text-slate-800' : 'text-slate-300'}`}>
                <CheckCircle2 size={18} className={progress >= 60 ? 'text-[#38A3A5]' : 'text-slate-200'} />
                <span>Parse Clinical Notes</span>
              </div>
              <div className={`flex items-center gap-3 text-sm font-semibold transition-all duration-500 ${progress >= 100 ? 'text-slate-800' : 'text-slate-300'}`}>
                <CheckCircle2 size={18} className={progress >= 100 ? 'text-[#38A3A5]' : 'text-slate-200'} />
          ...
          <div className="flex-1">
            ...
            {loading ? (
               <div className="h-64 flex flex-col items-center justify-center space-y-4">
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Aligning EHR Network</p>
               </div>
            ) : (
               <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
                  <DataField label="Patient Name" value={caseData?.patient_name} />
                  <DataField label="Primary Insurance" value={caseData?.insurance_company} />
                  <DataField label="Ordering Physician" value={caseData?.physician_name} />
                  <DataField label="Diagnostic Labs" value={caseData?.lab_results} isLabs={true} />
               </div>
            )}
          </div>
