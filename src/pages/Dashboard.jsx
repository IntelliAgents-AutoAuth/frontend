      ...
      <aside className="hidden lg:flex w-72 bg-white border-r border-slate-100 flex-col fixed inset-y-0 z-20 overflow-y-auto scrollbar-hide">
        ...
        <nav className="flex-1 px-4 space-y-1.5 focus:outline-none">
          ...
          <div className="mt-12 px-2 pt-8 border-t border-slate-50 opacity-40">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">Global Gateway v4</p>
          </div>
        </nav>
      ...
      <main className="flex-1 lg:ml-72 flex flex-col min-h-screen min-w-0 bg-[#FDFDFD]">
        <header className="h-24 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center sticky top-0 z-10 w-full">
          <div className="max-w-[1600px] mx-auto w-full flex items-center justify-between px-6 lg:px-12">
            <div className="min-w-0">
              ...
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1 truncate">Status: Operational • Region: US-EAST-1</p>
            </div>
        ...
        <div className="max-w-[1600px] mx-auto w-full flex-1 flex flex-col">
          <section className="p-6 lg:p-12 space-y-8 lg:space-y-10 fade-in flex-1 min-w-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-slate-400">Database Index</h2>
              </div>
              <div className="flex items-center gap-3">
                <button className="btn-secondary py-2 px-5 text-xs whitespace-nowrap">
                  <Filter size={14} className="text-[#38A3A5]" />
                  Filter Dataset
                </button>
              </div>
            </div>
                      ...
                      <tr>
                        <td colSpan="5" className="px-10 py-24 text-center">
                          <div className="flex flex-col items-center justify-center gap-4 text-slate-400">
                            ...
                            <p className="font-bold text-xs uppercase tracking-widest">Synchronizing Clinical Data...</p>
                          </div>
                        </td>
                      </tr>
                    ) : (
                        ...
                        <tr key={c.case_id} className="group hover:bg-slate-50/80 transition-all cursor-pointer border-b border-slate-50 last:border-0" onClick={() => navigate(`/cases/${c.case_id}`)}>
                          <td className="px-6 sm:px-10 py-5 sm:py-7">
                            <div className="flex flex-col gap-1.5">
                              ...
                              <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap hidden sm:block">{c.created_at}</p>
                            </div>
                          </td>
                          <td className="px-6 py-5 sm:py-7">
                            <div className="flex flex-col">
                              <p className="font-bold text-slate-800 text-sm h-5 flex items-center mb-2">{c.patient_name}</p>
                              <div className="2xl:hidden flex flex-col gap-1">
                                <p className="text-[10px] sm:text-[11px] font-bold text-[#38A3A5] uppercase tracking-wider h-4 flex items-center">{c.procedure}</p>
                                <p className="text-[9px] font-medium text-slate-400 uppercase tracking-tighter truncate">CPT: {c.cpt_code} • {c.insurance}</p>
                              </div>
                          ...
                          <td className="px-10 py-5 sm:py-7 hidden 2xl:table-cell">
                            <div className="flex flex-col gap-2">
                              <p className="font-bold text-slate-700 text-sm leading-none h-4 flex items-center">{c.procedure}</p>
                              <div className="flex">
                          ...
                          <td className="px-6 py-5 sm:py-7">
                            <div className="flex items-center h-full">
                              <span className={`inline-block px-3 sm:px-4 py-1.5 rounded-xl text-[9px] sm:text-[10px] font-bold uppercase tracking-widest border whitespace-nowrap ${STATUS_MAP[c.status]?.color || 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                                {STATUS_MAP[c.status]?.label || 'DRAFT'}
                              </span>
                            </div>
                          </td>
          ...
          <footer className="py-8 px-6 lg:px-12 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6 text-[10px] font-bold tracking-[0.2em] text-slate-400 uppercase bg-white/50 backdrop-blur-sm mt-auto">
            <p className="text-center sm:text-left">© 2026 AutoAuth Private Cloud • Distributed Infrastructure</p>
            <div className="flex gap-6 lg:gap-10">
              <a href="#" className="hover:text-[#38A3A5] transition-colors">Audit Ledger</a>
              <a href="#" className="hover:text-[#38A3A5] transition-colors">Privacy Protcol</a>
            </div>
          </footer>
        </div>
      </main>
